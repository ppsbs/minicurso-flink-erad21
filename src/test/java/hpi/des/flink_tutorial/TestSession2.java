...
public class TestSession2 extends TestCase {
    ...
    public void testExercise6(){
        try {
            Duration interval = EventTimeTipInvestigation.exercise6WatermarkInterval();
            SerializableTimestampAssigner<TaxiRideTuple> assigner = EventTimeTipInvestigation.exercise6GetTimestampFromTaxiRideTuple();
            TaxiRideTuple ride = new TaxiRideTuple("1", this.dateTime1, this.dateTime2, 1, 1.20, 3, "N", 41, 24, 2, 5.5, 0.5, 0.5, 0.0, 0.0, 0.3, 6.8, 0.0);

            assertEquals(DateParser.localDateTimeToMilliseconds(ride.f1), assigner.extractTimestamp(ride, -42L));
            assertEquals(interval.toMinutes(), 10L);
        }
        catch (Exception e){
            e.printStackTrace();
            fail();
